...
class OtController extends Controller
{
    ...
    {
        $data['id'] = $id;
        $data['banner'] = OtBanner::where('pages_id', $id)->first();
        $data['content'] = FeatureService::with('subContents')->where('pages_id', $id)->first();
        return view('admin.hospital_ot.create', $data);
    }
    ...
    public function store(Request $request)
    {
        ...
        if ($request->banner_id) {
            $banner = OtBanner::findOrFail($request->banner_id);
        ...
        if ($request->hasFile('image')) {
            ...
            $destinationPath = public_path('images/ot/banner');
            ...
            $banner->image = 'images/ot/banner/' . $imageName;
        }
        ...
        return redirect()->route('admin.pages.index')
            ->with('success', 'Hospital Ot saved successfully.');
    }
    ...
    public function contentStore(Request $request)
    {
        ...
        if ($request->content_id) {
            ...
            $content = FeatureService::findOrFail($request->content_id);
        ...
        if ($request->hasFile('images')) {
            ...
            $destinationPath = public_path('images/service/content');
            ...
            $content->update([
                'image' => 'images/service/content/' . $imageName
            ]);
        }
        ...
        foreach ($request->heading as $index => $heading) {
            ...
            if ($subId) {
                ...
                // UPDATE EXISTING
                FeatureSubService::where('id', $subId)->update([
                    'heading'     => $heading,
                    'description' => $request->sub_description[$index],
                ]);
            } else {
                // CREATE NEW
                FeatureSubService::create([
                    'feature_services_id'    => $content->id,
                    'heading'                => $heading,
                    'description'            => $request->sub_description[$index],
                ]);
            }
